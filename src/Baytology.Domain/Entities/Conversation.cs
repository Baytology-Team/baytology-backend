...
public sealed class Conversation : Entity
{
    ...
    public static Result<Conversation> Create(Guid propertyId, string buyerUserId, string agentUserId)
    {
        ...
        var normalizedBuyerUserId = buyerUserId.Trim();
        var normalizedAgentUserId = agentUserId.Trim();

        if (string.Equals(normalizedBuyerUserId, normalizedAgentUserId, StringComparison.Ordinal))
            return ConversationErrors.ParticipantsMustDiffer;

        return new Conversation(propertyId, normalizedBuyerUserId, normalizedAgentUserId);
    }
