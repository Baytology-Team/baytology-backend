...
namespace Baytology.Domain.ValueObjects;
...
public sealed class RecommendationResult : Entity
{
        ...
        decimal? snapshotPrice)
    {
        ...
        var normalizedRecommendedPropertyId = recommendedPropertyId == Guid.Empty ? null : recommendedPropertyId;
        var normalizedExternalReference = string.IsNullOrWhiteSpace(externalReference) ? null : externalReference.Trim();

        if (!normalizedRecommendedPropertyId.HasValue && normalizedExternalReference is null)
        ...
        return new RecommendationResult(
            requestId,
            normalizedRecommendedPropertyId,
