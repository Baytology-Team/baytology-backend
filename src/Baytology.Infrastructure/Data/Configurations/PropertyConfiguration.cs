    ...
    public void Configure(EntityTypeBuilder<Property> builder)
    {
        builder.ToTable("Properties", table =>
        {
            table.HasCheckConstraint(
                ...
                "[Price] > 0 AND [Price] >= 1000 AND [Price] <= 999999999 AND " +
                "[Area] > 0 AND [Area] >= 10 AND [Area] <= 100000 AND " +
                "[Bedrooms] >= 0 AND [Bedrooms] <= 100 AND " +
                "[Bathrooms] >= 0 AND [Bathrooms] <= 100 AND " +
                "([Floor] IS NULL OR ([Floor] >= 0 AND [Floor] <= 999)) AND " +
                "([TotalFloors] IS NULL OR ([TotalFloors] > 0 AND [TotalFloors] <= 999)) AND " +
