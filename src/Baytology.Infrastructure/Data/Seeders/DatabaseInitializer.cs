...
public static class DatabaseInitializer
{
            ...
            if (!await roleManager.RoleExistsAsync(role))
            {
                ...
                if (!result.Succeeded)
                {
                    throw new InvalidOperationException(
                        $"Failed to create role '{role}': {FormatIdentityErrors(result.Errors)}");
                }
    ...
    private static async Task SeedAdminUserAsync(IServiceProvider services, ILogger logger)
    {
        ...
        if (adminUser is null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Failed to add admin user to role '{Role.Admin}': {FormatIdentityErrors(roleResult.Errors)}");
                }
            ...
            {
                throw new InvalidOperationException(
                    $"Failed to create admin user: {FormatIdentityErrors(result.Errors)}");
            }
        }
        else
        {
            ...
            if (!await userManager.CheckPasswordAsync(adminUser, adminPassword))
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Failed to synchronize admin user password: {FormatIdentityErrors(passwordResult.Errors)}");
                }
            }
            ...
            if (!await userManager.IsInRoleAsync(adminUser, Role.Admin))
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Failed to add existing admin user to role '{Role.Admin}': {FormatIdentityErrors(roleResult.Errors)}");
                }
    ...
    private static async Task NormalizeAgentIdentityAsync(IServiceProvider services, ILogger logger)
    {
        ...
        foreach (var userId in usersWithActiveAgentResponsibilities.Where(userId => !agentRoleUserIdSet.Contains(userId)))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Failed to normalize Agent role for user '{userId}': {FormatIdentityErrors(addRoleResult.Errors)}");
            }
